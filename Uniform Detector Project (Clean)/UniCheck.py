...
class MultiCamApp:
        ...
        while getattr(self, f'{cam_name}_running'):
            ...
            if ret:
                ...
                if cam_name == "cam1":                      #Limit the uniform detection to camera 1 only
                    ...
                    if (self.remarks != '') and (bool(self.qr_data_dict)):
    ...
    def capture_cam1(self):
        if self.cam1_frame is not None:
            ...
            #Send email only if not in uniform
            if self.remarks == 'Not_In_Uniform':
...
class ObjectDetector:
    ...
    def detect(self, frame):
        ...
        self.label_annotator3 = sv.LabelAnnotator(text_position=sv.Position.TOP_CENTER,text_scale=0.5,text_thickness=1,text_padding=10,color=sv.Color(255, 165, 0))
        self.box_annotator3 = sv.BoxCornerAnnotator(thickness=2,color=sv.Color(255, 165, 0))    #Orange
        ...
        labels = [f"{class_name} {confidence:.2f}" for class_name, confidence in zip(mapped_class_names, detections.confidence)]
        ...
        if all(p == 'notInUniform' for p in detections['class_name']):
            ...
            return frame , 'Not_In_Uniform'
        if all(p == 'pe-ntstp' for p in detections['class_name']):
            ...
            return frame , 'PE_NSTP'
        if all(p == 'uniforms' for p in detections['class_name']):
            frame = self.label_annotator2.annotate(scene=frame, detections=detections, labels=labels)
            frame = self.box_annotator2.annotate(scene=frame, detections=detections)
            return frame , 'In_Uniform'
        if all(p == 'person' for p in detections['class_name']):
        ...
        if "notInUniform" in detections['class_name']:
            ...
            frame = self.box_annotator1.annotate(scene=frame, detections=detections)
            return frame , 'Not_In_Uniform'
        if "uniforms" in detections['class_name']:
            frame = self.label_annotator2.annotate(scene=frame, detections=detections, labels=labels)
            frame = self.box_annotator2.annotate(scene=frame, detections=detections)
            return frame , 'In_Uniform'
        if "pe-ntstp" in detections['class_name']:
            frame = self.label_annotator2.annotate(scene=frame, detections=detections, labels=labels)
            frame = self.box_annotator2.annotate(scene=frame, detections=detections)
            return frame , 'PE_NSTP'
...
class EmailHandler:
    def __init__(self):
            # Example usage
        self.sender_email = 'djtiglao@cca.edu.ph'
        self.subject = 'Test Email'
        self.body = 'This is a test email sent using Python.'
    ...
    def send_email(self, receiver_email, image_path):
        ...
        try:
            with smtplib.SMTP('smtp.gmail.com', 587) as smtp_server:
                ...
                smtp_server.login(self.sender_email, 'ewua lhvi hoqq slot')  # Use your app password here
